    ...
    def epoch(self, examples: torch.LongTensor):
        ...
        with tqdm.tqdm(total=examples.shape[0], unit='ex', disable=not self.verbose) as bar:
            ...
            while b_begin < examples.shape[0]:
                ...
                pos = input_batch
                neg = pos.clone()
                neg[:, 2] = torch.randint(0, self.model.sizes[0], (neg.size(0),), device=neg.device)

                pos_score = self.model.score(pos)
                neg_score = self.model.score(neg)

                # softplus 损失
                # l_fit = (F.softplus(-pos_score) + F.softplus(neg_score)).mean()
